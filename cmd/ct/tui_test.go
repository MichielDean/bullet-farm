...
// ── execIssueActionCmd: resolve and reject ────────────────────────────────────

// TestExecIssueActionCmd_ResolveIssue_ResolvesIssue verifies that
// execIssueActionCmd with actionResolveIssue marks the issue as resolved.
//
// Given: a real cistern DB with a droplet and an open issue
// When:  execIssueActionCmd with actionResolveIssue is executed
// Then:  tuiActionResultMsg.err is nil and the issue status is "resolved"
func TestExecIssueActionCmd_ResolveIssue_ResolvesIssue(t *testing.T) {
	dbPath, id := newTestDBWithDroplet(t)
	...
	m := newTabAppModel("", dbPath)
	m.selectedID = id
	cmd := m.execIssueActionCmd(iss.ID, actionResolveIssue, "fixed it")
	msg := cmd()
...
// TestExecIssueActionCmd_RejectIssue_RejectsIssue verifies that
// execIssueActionCmd with actionRejectIssue marks the issue as unresolved.
//
// Given: a real cistern DB with a droplet and an open issue
// When:  execIssueActionCmd with actionRejectIssue is executed
// Then:  tuiActionResultMsg.err is nil and the issue status is "unresolved"
func TestExecIssueActionCmd_RejectIssue_RejectsIssue(t *testing.T) {
	dbPath, id := newTestDBWithDroplet(t)
	...
	iss, err := c.AddIssue(id, "test-flagger", "still broken")
	c.Close()
	if err != nil {
		t.Fatal(err)
	}
	...
	m := newTabAppModel("", dbPath)
	m.selectedID = id
	cmd := m.execIssueActionCmd(iss.ID, actionRejectIssue, "still an issue")
	msg := cmd()
