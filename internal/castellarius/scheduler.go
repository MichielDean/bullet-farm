...
func (s *Castellarius) heartbeatRepo(ctx context.Context, repo aqueduct.RepoConfig) {
	...
	for _, item := range items {
		...
		if item.Assignee == "" {
			// No assignee — assigned_aqueduct not persisted (see ci-dko75).
			// Debounce stays set so we do not spam this warning every tick.
			s.logger.Warn("heartbeat: stall detected but no assignee — cannot respawn",
				"droplet", item.ID,
			)
		} else if err := s.respawnStalledDroplet(ctx, client, repo, item); err != nil {
