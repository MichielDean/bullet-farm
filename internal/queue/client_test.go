...
func TestAssign(t *testing.T) {
	...
	if got.CurrentStep != "implement" {
		t.Errorf("current_step = %q, want %q", got.CurrentStep, "implement")
	}
...
func TestAssign_EmptyWorker_SetsOpen(t *testing.T) {
	...
	if got.CurrentStep != "review" {
		t.Errorf("current_step = %q, want %q", got.CurrentStep, "review")
	}
}
...
func TestAddNote_And_GetNotes(t *testing.T) {
	...
	if notes[0].StepName != "implement" || notes[0].Content != "wrote the code" {
		t.Errorf("note[0] = %+v", notes[0])
	}
	if notes[1].StepName != "review" || notes[1].Content != "looks good" {
